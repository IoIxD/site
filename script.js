...
document.addEventListener("mousemove", function(e) {
  if(mouseDown == 1) {
    elemHover = document.querySelectorAll(`:hover`);
    if(elemHover.length >= 3 && elemHover[2].classList[0] == "window") {
      // yeah for some reason it sometimes forgets mouseDown is a thing so this is a sloppy fix to correct that
      if(elemHover[2].tagName == "iframe") {mouseDown = 0;}
      elemHover[2].style.top = (+(e.pageY-my_o) + +wy_o)+"px";
      elemHover[2].style.left = (+(e.pageX-mx_o) + +wx_o)+"px";
    }
  }
})
...
function OpenTheThree() {
  windowCreate('main', '500px', '300px', '50%', '200px', 'textfile');
